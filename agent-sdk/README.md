...
**`npm install @stateofintent/agent-sdk` gives you a working SDK as of 2026-09-12.** `0.1.2` is on the registry and is `latest`, published by `scripts/publish-packages.sh --publish`, which runs the packaging checks and a live check against a real conductor before it will spend a version number. Verified afterwards from the registry rather than from the tarball that was uploaded: installed by version into an empty project outside this checkout, it connects, authorizes signing, writes a claim the network accepts, and reads it back.

**`0.1.1` is still on the registry and is still broken**, so pin or range above it — `mcp-server` requires `^0.1.2` for exactly this reason. Nothing resolves `0.1.1` by default now that `latest` has moved, but an explicit `0.1.1` still gets the build described below.

The detail of what was wrong with `0.1.1`, kept because it is the clearest example in this repository of a package that passes every packaging check and still does not work: It predates the `@holochain/client` 0.21 upgrade, where `CellInfo` became a discriminated union (`{ type, value }`) rather than an object keyed by cell type. The published build still tests `CellType.Provisioned in cell`, which compiles fine against the new client — these values are `any` — and matches nothing at runtime: no cell ids are collected, `authorizeSigningCredentials` runs over an empty list, and the first real zome call fails with `NoSigningCredentialsForCell`. It also reads `record.signed_action.hashed.content.entry_hash`, one level shallower than the current `…content.data.entry_hash`. Both were fixed here and are now released in `0.1.2`. This was found by building `mcp-server` against the registry copy and watching its live harness go red on eight checks; the same harness is green against this tree, and green again against `0.1.2` installed from the registry.

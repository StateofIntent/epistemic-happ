...
/// `2^(-elapsed_secs / half_life_secs)`, the same shape decay_factor
/// already uses, parametrized rather than sharing its fixed constant —
/// WeightedAttestationPolicy lets a caller name their own half-life per
/// call, unlike conductance decay which is a fixed, protocol-wide
/// constant. Non-positive elapsed
...
mod tests {
    ...
    // --- epoch_decay_factor ------------------------------------------------
    //
    // Same 2^(-elapsed/half_life) shape decay_factor above already has its
    // own dedicated tests for; these confirm the parametrized instance
    // behaves identically rather than re-testing the formula from scratch.

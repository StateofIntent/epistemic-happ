...
Scope has grown well past the first increment: browsing and publishing `Claim`s, viewing and adding `Critique`s, Membranes and their discourse health, evidence and retraction, spatial navigation of the critique tree, the critique taxonomy, and — as of the Notes tab — the shared notes layer that sits *above* the protocol, with the promotion gate between them. The worldline half of HRR now has a Worldline tab; **neighborhood binding does not**, and §2.5 treats the two as independent use cases rather than halves of one job. The root `README.md` §9 roadmap item carries the current count and the itemised residue; this sentence is a summary of it and not a second source of truth — which is exactly how it went stale, by being written as though it were one.
...
**One constraint on the Worldline tab, and it is the reason that screen is shaped the way it is.** `period_boundaries` is the **exact, lossless** record; HRR resonance is **approximate by construction**, and the coordinator says a high similarity "is a hint worth checking, not a claim of fact" and "never substitutes" for the boundaries. So the exact record renders first and unconditionally, the probe sits beneath it, every hit names the boundary it points at, and `sample_period` opens that window's real records — the hint has to stay checkable. Two further things are easy to get wrong and one of them was: `query_worldline_resonance` scores **every** period and applies no threshold, so an unrelated tag still returns a full ranked list (a real tag scored ~0.71 against a nonsense tag's −0.09), and presenting that as *matches* turns a ranking into a verdict through layout alone. And the probe is deliberately limited to your own worldline — probing others would produce a per-agent scalar that `get_membrane_members` makes sortable, which is the leaderboard the protocol removed `get_credit_balance` to avoid.
...
**One constraint on the colour theme, which is the only piece of chrome here that a user can pin.** The palette follows the OS by default and that default stays — `system` is a live deferral to `prefers-color-scheme`, not a third fixed palette, so an OS change while the app is open still repaints it. What a pin has to do is win in *both* directions: the obvious implementation (`:root[data-theme='dark']` and nothing else) makes pinning dark on a light desktop work and pinning light on a dark one do nothing, because the media query is still in charge. Hence the `:not([data-theme='light'])` guard on that query, and hence the two dark token blocks in `style.css` that must be edited together. A pin also has to move `color-scheme`, or the browser keeps painting scrollbars and carets from the OS preference the user just overrode. `index.html` carries a small pre-paint read of the same key for one reason: the module bundle is deferred, so leaving the read to `src/theme.ts` alone paints one light frame per launch — the white flash the pin exists to prevent. `scripts/live-verify/theme-pinning.mjs` asserts all of it under both emulated OS preferences, and is the one harness in that directory that needs no conductor, because the control sits in the title row and chrome is not gated on being connected.

**One constraint on the Notes tab, which is the only screen here that talks to something other than a conductor — and the only one that carries the gate.** The notes layer (`notes/`) is a separate service holding no Holochain credentials, so promotion is performed *by this client*, under the practitioner's own agent key, and the service is told afterwards. Three things follow and each is asserted by `scripts/live-verify/notes-ui.mjs`. The tab is reachable **without a conductor at all** — the connect screen offers a second door, because requiring a connection first would put the protocol's ceremony back in front of the room built to sit in front of the protocol. The promotion form is therefore *rendered and disabled with a reason* when there is no connection, never hidden: it is unavailable now, not structurally impossible, which is exactly §4.5's rule and exactly the distinction `affordance-surfacing.mjs` guards elsewhere. And promotion is a **copy, not a move** — the note stays where it is, and records the `ActionHash` of what it became. Watched failing on both: hiding the form turns four checks red, deleting the note after publishing turns three red while the DHT checks correctly stay green.

**One constraint on the Notes tab that only exists because the room went live.** The tab now long-polls `GET /spaces/:id/events` while a space is open, so another member's note appears without anybody touching this screen — until it did, two people in one notebook wrote past each other invisibly, and no harness could see it because each drove a single client. Making the screen move by itself creates three failures that were previously unreachable, and each is a rule rather than a detail. **A draft never lives in the DOM**: `render()` rebuilds this app from scratch, so a half-typed sentence in a textarea is destroyed by a stranger's note arriving — composer and editor drafts are held in module state, exactly as `promotionDraft` already was and for exactly the same reason. **The caret is restored across the rebuild**, because moving somebody's cursor mid-word is the same defect wearing better clothes. And **a save states which version it was written against**, so a note that moved under an open editor is refused with a 409 rather than silently overwriting the other person, with both versions shown and the choice left to the reader. One further constraint is not about this screen at all: the client **hangs up on its poll when it leaves a room or the tab**, because the service caps concurrent parked polls per member, and a browser that wanders off holding sockets spends a ceiling that exists to bound machines. `scripts/live-verify/notes-live.mjs` drives two browser contexts and asserts every clause; it was watched failing when the composer draft was moved back into the textarea (two red, arrival still green — the room was live, and being live is what ate the sentence).

**One constraint on the assistant in that form, which is the only place in this app where a model's output reaches a screen.** The design's phrasing is the specification: an assistant that can say "this reads like a methodological critique — want me to draft it that way?" and be "right often enough to be useful and wrong safely enough to be corrected". Three things make the second half true. The assistant is **a member of the space**, so the button is offered only when the room actually contains one — not a service-level helper nobody in the room can see or remove. Its answer is **attributed and labelled**: who suggested it, and what produced it, in the answerer's own words, because "a model said so" and "a keyword table said so" are different claims and the reader is the one deciding. And **nothing is applied on arrival** — every field lands beside a button somebody presses, and the form is complete and usable with the assistant ignored, since the design is explicit that someone who wants to explore unaided must still be able to. `scripts/live-verify/notes-assistant.mjs` asserts all three, and its auto-apply injection **passed the first time** — the check compared the mode field against a default that happened to equal the suggestion. Strengthened, it turns two checks red, and the strengthening then found a real defect: asking the assistant used to wipe the form, because a re-render rebuilt fields initialised from the note. Hence `promotionDraft`.

**And one constraint carried up from the service: nothing on that screen may become comparative.** The directory's ordering control offers `recent` and `alphabetical` and there is no third value to add — the service refuses anything else with a 400 rather than falling back, and the client's own type has two members so the refusal is unreachable from here. Activity is rendered as a *sentence* about one room ("12 notes this week · 3 people active today") rather than a row of figures, because a number in a box invites comparison with the same box elsewhere. Participant count is shown, since a newcomer needs to know whether anyone is there, and is never used to order anything. This is Invariant #1 one layer up, and it arrives disguised as a convenience every single time.

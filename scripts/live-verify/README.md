...
**Which Chromium.** The browser harnesses were written on a machine with a system Chromium at `/usr/bin/chromium`, and the ones needing a conductor still name that path outright — they only ever run where a Holochain toolchain is already installed. The three conductor-free browser harnesses resolve it instead: `EPI_CHROMIUM` if set, then `/usr/bin/chromium` if it exists, then Playwright's own download. That last case is what a CI runner uses.
...
**And four harnesses from this directory now run against a real conductor on every push** — `.github/workflows/conductor.yml`: `domain-index`, `read-scope`, `mew-lifecycle` and `friction-limits`, each preceded by `sandbox.sh clean && sandbox.sh start`, exactly as the rule above demands. This is the first CI here that is evidence about the protocol rather than about the layer above it.
...
Still not running there, and none of it is a wall: the browser harnesses below the gate (they hardcode `/usr/bin/chromium` — see the Chromium note below, whose reasoning expires now that a CI conductor exists), `agent-sdk`/`mcp-server`/`linked-data-gateway` (each needs its own package built first), and the four multi-node ones, whose bootstrap and iroh-relay binaries are published on the same release tag.

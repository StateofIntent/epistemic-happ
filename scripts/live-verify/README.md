...
**Three harnesses need no conductor at all**, and are the only ones exempt from the rule above: `theme-pinning` drives the built UI bundle alone, and `notes-layer` and `notes-assistant` drive the `notes/` service, which lives *above* the promotion gate and by construction never touches a DHT. All three are safe to run at any time, in any order, alongside anything else. Both notes harnesses need `cd notes && npm install && npm run build` first (and `notes-assistant` a built UI as well) — the same "packing does not compile" trap as everywhere else, in a different package.
...
| `notes-assistant` | browser, `notes/` built, **no conductor** | 2 members + **1 AI member** | The AI in the room — it joins through an ordinary invite link as a visible member, its answer is attributed and labelled with what produced it, and nothing it suggests is applied until a button is pressed |

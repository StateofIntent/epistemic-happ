...
**And five harnesses from this directory now run against a real conductor on every push** — `.github/workflows/conductor.yml`: `domain-index`, `read-scope`, `mew-lifecycle`, `friction-limits` and `notes-ui`, each preceded by `sandbox.sh clean && sandbox.sh start`, exactly as the rule above demands. This is the first CI here that is evidence about the protocol rather than about the layer above it — and `notes-ui` means **the promotion gate itself is no longer something only a person can check**.
...
Still not running there, and none of it is a wall: the other seventeen conductor-bound browser harnesses (they still hardcode `/usr/bin/chromium` — see the Chromium note below), `agent-sdk`/`mcp-server`/`linked-data-gateway` (each needs its own package built first), and the four multi-node ones, whose bootstrap and iroh-relay binaries are published on the same release tag.

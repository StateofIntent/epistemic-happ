...
**Which Chromium.** The browser harnesses were written on a machine with a system Chromium at `/usr/bin/chromium`, and seventeen of the conductor-bound ones still name that path outright. The three conductor-free browser harnesses resolve it instead — `EPI_CHROMIUM` if set, then `/usr/bin/chromium` if it exists, then Playwright's own download — and `notes-ui` now does too, because `conductor.yml` runs it on a machine that has a conductor and no system browser.

**The argument for leaving the other seventeen alone has changed shape, and is worth stating precisely.** It used to be that a conductor-bound harness only ever ran where a system Chromium already was, so resolving the path was unexercisable churn. That premise is gone. What replaces it is narrower and still holds: **change the resolution in the harness you are adding to CI, in the same commit that adds it**, so the edit is exercised by the check that motivated it. Seventeen files edited at once to satisfy a workflow running one of them would be the same churn wearing a better justification.

**Four of these run on every push and pull request** — see `.github/workflows/live-verify.yml`. They are exactly the conductor-free ones described in the next paragraph, and nothing else is smuggled in behind them: the rest need a Holochain toolchain, a built WASM DNA and a clean conductor per run, and a green tick that quietly stopped covering them would be worse than no tick. In particular `notes-ui.mjs`, which covers the promotion gate itself against a real DHT, is still something a person runs on a machine with a sandbox. CI is evidence about the layer above the gate and says nothing about the protocol below it.
...
**And two of the multi-node harnesses run there too** — `.github/workflows/network.yml`, a separate job because it needs a different setup: `real-gossip` and `transitive-gossip`, each preceded by `network.sh clean && network.sh start`, which is the same hard rule as above and just as observed. This is the only automatic evidence in the repository about propagation between peers; `conductor.yml`'s header says in as many words that a green tick there is silent on gossip, partition and convergence, because it is one conductor on one machine.
...
Still not running there, and neither is a wall: the other seventeen conductor-bound browser harnesses (they still hardcode `/usr/bin/chromium` — see the Chromium note below), and the two remaining multi-node ones — `partition-rejoin`, at about five and a half minutes and driving `stop-node`/`start-node` itself, and `network-partition`, at about twenty-five minutes inside a privileged throwaway namespace. Both costs are measured rather than guessed, which is the difference between this list and the one it replaces.

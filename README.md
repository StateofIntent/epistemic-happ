...
The relation vocabulary used above (`asserted by`, `has domain`, `critiques`, `retracts`, etc.) is registered in [`n4l/arrows-epistemic.sst`](../n4l/arrows-epistemic.sst) at the repo root, organized under N4L's `leadsto`/`contains`/`properties` categories. That file must be loaded into `SSTconfig/` alongside SSTorytime's own standard arrow files before ingesting any exported `.dat` file.

**⚠️ Verification status:** this output has been checked by hand against the published N4L language spec, but has **not** been run through the real `N4L` Go binary (`N4L -v somefile.dat`). "Matches the spec as written" and "actually parses" are different claims until someone with the SSTorytime toolchain installed verifies it end-to-end. Treat the exporter as spec-conformant-but-unverified until that check happens — see Section 9 (Roadmap) for this as an open item.
...
- [ ] **Verify `export_to_n4l` output against the real `N4L` Go binary** (`N4L -v somefile.dat`) — output is hand-checked against the published spec but not yet parser-verified; this is the current blocking unknown for this phase
- [ ] Local SSTorytime instance per agent
- [ ] N4L ingestion pipeline
- [ ] 3D graph visualization of critique manifold
- [ ] Local cone paths navigation

### Phase 3: HRR Implementation (Deferred)
- [ ] Local HRR compressor (OpenZoo math)
- [ ] Vector binding/unbinding protocol
- [ ] Populate `trace_payload` and `binding_key`
- [ ] Peer HRR query support

...
- [ ] **Token/cost layer (burn-to-deploy for domains) — evaluated and left explicitly out of scope for now, not silently skipped.** §2.3 originally describes this as SWO's own burn-to-deploy pattern (an Arbitrum Orbit L3 mechanism) applied to domain creation. A *real* version of that presupposes an actual token/currency, and Holochain has no native token or gas concept at the protocol level the way an Arbitrum L3 does — this codebase has zero existing token/currency scaffolding to build on, unlike, say, the Twitter bridge, which at least has a real (if credential-blocked) integration skeleton. Building this honestly would mean either standing up a whole internal fungible-token zome (mint/burn/balance tracking, with no connection to any real SWO/Arbitrum value — a materially bigger, more speculative undertaking than anything else in Phase 4) or bridging to an actual external chain (real deployed contracts, wallet integration — further still). Neither was attempted. This is also consistent with a decision this design already made on purpose: `Membrane`'s own doc comment states creating a domain was made *accountable* (a real `Constitution` + `required_promises`, §2.6) rather than *costly* — deliberately not a cryptoeconomic layer, the same distinction §2.3's "Why identity creation was never made to cost something, on purpose" note draws for agent identity. Revisiting this needs its own explicit scoping decision (internal token vs. real external bridge) before any code gets written, not a default "build something token-shaped" pass.

...
- [ ] **THE PRIMARY BROWSE PATH IS CHAIN-LOCAL — five reads cannot see other agents at all.** Found while scoping the critique-taxonomy UI, proven with two real agents, and the most consequential defect this project has surfaced to date. Now specified in `SPEC.md` §10.0.
  ...
  **Not fixed here, and the fix is real work rather than a patch.** The shape is half-present already: `LinkTypes::MembraneToClaim` is declared in the integrity zome and — as PR #44's own SPEC audit recorded, without drawing this conclusion from it — is never created or read by any coordinator function. A domain-anchored link written at `create_claim` time and read by `get_claims_by_domain` is the obvious candidate, but it needs a validation rule, a decision about whether the anchor is the domain string or the `Membrane`, and a story for claims already published without one. That is a DNA change with a migration question attached, not a UI pass.

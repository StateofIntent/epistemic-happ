...
- [ ] **Surface the epistemic state the backend already computes — the UI calls 4 of 56 coordinator functions.** `mobile-ui/` currently uses `create_claim`, `create_critique`, `get_claims_by_domain` and `get_critiques_for`, and nothing else. Everything this protocol computes *about* discourse — the part that distinguishes it from a forum — is invisible. This is the highest-leverage UI work available, and it needs no new zome code: every function below already exists, is specified in `SPEC.md`, and returns real data today.

  Grouped by what a practitioner would actually do with them:

  - **Epistemic state (the "HUD" layer).** `get_discourse_health` (critique-mode distribution and totals), `get_effective_conductance` (decay- and reinforcement-weighted strength of a critique's connection), `get_cross_domain_critiques` (which critiques came from outside the domain), `get_antibody_patterns_for` (flagged bad-faith patterns on a target), `get_critiques_by_mode`. A claim with contradictory unresolved critiques should not look like an uncontested one; all the state needed to show that difference is already there.
  - **`get_synaptic_link_friction_status` is the clearest single example.** It returns `recent_count`, `limit`, `window_secs` and `blocked` — a resource meter, already computed, currently shown nowhere. A user who hits the 20/hour cap today gets an opaque error instead of having watched a budget deplete.
  - **Membranes and governance.** `get_membranes`, `get_membrane_members`, `join_membrane`, `create_membrane`, `publish_constitution`, `get_agent_constitution` — domains, their founding promises, and who belongs to them are entirely absent from the UI.
  - **Evidence and grounding.** `create_evidence`, `get_evidence`, `get_grounding_path` — whether a claim's chain reaches real evidence is a binary the protocol will answer and the UI never asks.
  - **Critique taxonomy.** `get_all_critique_species`, `get_critique_species_adoption_count`, `create_critique_species` — the evolving vocabulary of critique types, invisible.
  - **Retraction.** `create_retraction`, `get_retractions_for_claim` — an author standing down from a claim is a first-class act here and cannot currently be performed or seen.
  - **Opt-in trust lenses.** `is_agent_attested`, `grant_attestation` — exposed as lenses the user aims, per §4.4's first constraint, never as an inferred ordering.

  **Design guidance worth borrowing, and what not to borrow.** Game HUDs are the right reference for this work: they adapt because they read rich simulation state and decide what is worth showing, not because of any rendering technology. The transferable patterns are state-driven affordance surfacing, a local-first in-memory mirror of DHT reads so rendering never blocks on a zome call, spatial navigation of the critique graph (which SSTorytime already supports), and progressive disclosure for onboarding — this protocol has a lot of concepts and games are the best discipline in software at introducing complexity gradually. **What does not transfer** is immediate-mode canvas rendering, which would cost text selection, deep linking and screen-reader access — a real regression given `LumbarRehab` is the reference domain — and a continuous tick loop, since discourse is event-driven and mostly static. §4.4 states the two constraints that any adaptive interface here must respect.
- [x] **Agent SDK shipped — `agent-sdk/`, see its own README for the norms it encodes.** The other half of "get agents using this": an AI agent participating here does not want a UI, it wants the zome API, and `SPEC.md` already specifies that completely. So the library is deliberately not an abstraction over the protocol — it is typed payloads (a wrong field name fails *quietly*, deserialising in Rust rather than erroring client-side), the admin-auth connection flow that eight places in this repository currently duplicate, and **the protocol's norms made into an API surface**. That last is the part worth reading: an autonomous agent is the participant most able to violate these constraints at scale and least likely to notice, because the obvious engineering move is usually the forbidden one. So `critique()` requires a `CritiqueMode` and defaults none (Invariant #4 — defaulting to `Logical` would flatten the distribution `get_discourse_health` reports); trust lenses are passed exactly as supplied and never defaulted (§4.4); `budget()` lets an agent pace itself rather than discover the rate limit by hitting it, and `critique()` throws a typed `FrictionLimitError` carrying the budget so a loop can tell "wait" from "broken". **There is deliberately no ranking, scoring or sort-by-credibility method**, and the README says so explicitly with what to do instead — that absence is the first thing someone reaches for, and left unexplained they would simply build it themselves. `call()` stays public as an escape hatch, since the specification is the authority and a convenience layer should never be why something is unreachable. Verified live against a real conductor by `scripts/live-verify/agent-sdk.mjs`, including driving the friction budget to exhaustion to confirm the typed error and its payload. The worked example (`examples/critique-agent.mjs`) includes the part such examples usually skip: **abstaining** — an agent that critiques everything is a spam flood with a language model attached, and `AntibodyPatternKind::SpamFlood` exists for exactly that.

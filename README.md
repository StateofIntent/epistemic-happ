...
With a conductor up, `node scripts/live-verify/credit-transfer.mjs`
verifies the mutual-credit layer's countersigning flow end to end
against it. It installs a second agent on the same conductor itself
(countersigning needs both signers reachable on one network), so no
...
- [x] **Token/cost layer shipped — a metabolic biosignalling currency internal to the DHT, not the external-token or L3-bridge shape this line originally imagined; see `docs/metabolic-biosignalling-currency-brief.md` for the full research trail and design rationale.** No project token, no external tradable asset — value is a real, internal, entirely DHT-native mutual-credit ledger, which turned out to be buildable (the line above was right that an *external* token/L3-bridge version wasn't worth attempting, but wrong that "zero existing scaffolding" meant nothing internal was feasible). Two new integrity-zome entry types: `MutualCreditTransfer` (countersigned — both `from` and `to` must accept the same countersigning session, closing the real Holochain-specific double-spend risk holochain-open-dev/community-mutual-credit and a minimal implementation by vanarchist both document; a plain, non-countersigned commit is rejected outright by `validate`, checked against the raw `Entry::CounterSign` envelope before HDI's own `.flattened()` would otherwise discard it) and `CreditBurn` (single-signer — destroying your own standing needs no one else's consent). `get_credit_balance` (coordinator zome) replays an agent's own transfer-and-burn history with demurrage decay applied per individual transaction (Gesell's 1906 Freigeld design, tested at Wörgl 1932, still running as the Chiemgauer) — never a canonical, comparative balance ranking (Invariant #1): always one named agent's own standing, on request. Coupled to existing SWO friction: `SynapticLink` creation stays free up to the original limit, costs a real, DHT-verified `CreditBurn` between the free limit and a hard ceiling, and is rejected unconditionally at or above that ceiling regardless of any burn amount — a two-tier design correcting an unsafe "credit fully bypasses friction" idea found (and rejected) during research, since that would let anyone who accumulates enough credit defeat SWO's entire sybil-resistance purpose. The integrity zome independently re-derives every claimed burn via `must_get_agent_activity`, the same mechanism `count_recent_actions_since_checkpoint` already established — nothing about a burn is trusted from the client.
  **A "cascading" extension was researched and deliberately reshaped by Invariant #1, not built as first proposed.** SourceCred, EigenTrust, and MeritRank (arXiv 2207.09950) were all investigated as ways to make trust propagate transitively through the graph; a SourceCred/EigenTrust-style single canonical score per agent was ruled out immediately — that is exactly the "no karma, no trust index" Invariant #1 forbids. What got built instead is `WeightedAttestationPolicy`/`get_attestation_weight`, an extension of the *existing* `AttestationPolicy`/`count_attestations_pure` web-of-trust walk (Phase 1) adding transitivity decay (an attestation matters less the more hops it crossed) and epoch decay (it matters less the older it is) — but staying exactly as opt-in and caller-scoped as `AttestationPolicy` already is: every call names its own roots and decay preferences, nothing is stored, nothing is ranked across agents. **It borrows two of MeritRank's mechanisms; it is not MeritRank, and the docs brief's §2/§4.4 now say so precisely rather than leaving "MeritRank-inspired" to imply more than it should.** MeritRank defines *three* decay mechanisms — transitivity, connectivity, and epoch — and its own evaluation credits transitivity plus connectivity with delivering Sybil tolerance while finding that epoch decay *reduces* it. What shipped here keeps transitivity, omits connectivity, and adds epoch, so it has the shape without the property. That is survivable only because nothing here claims Sybil resistance (§2.3 treats that as an open problem addressed nowhere) — but it means no future mechanism should lean on this function for it. It is also unnormalized (a decayed reachability sum, not a PageRank-family score) and order-dependent (the `visited` cycle guard, harmless when counting distinct attesters, makes a weighted sum depend on link iteration order). Unit-tested against a fixture graph only; unlike the credit flow, it has never been run against a real conductor.
  Circles UBI's live, ~200k-account trust-graph pathfinder (multi-hop transitive payment routing) and hREA (an existing, real, multi-year Holochain hApp for REA/ValueFlows economic coordination, explicitly built for mutual-credit networks) were both researched as real, working precedents but **not built or integrated this pass** — see the docs brief for the reasoning (multi-hop atomic settlement without global consensus is its own hard distributed-systems problem; hREA integration would mean either becoming multi-DNA or vendoring a separate project's zomes, a bigger architectural commitment than this pass's scope). Physarum polycephalum's flux-coupled tube-adaptation model was also researched as a possible refinement to the existing `decay_factor` (making decay rate itself respond to traversal load, not just elapsed time) but left unbuilt — a refinement to already-shipped Phase 1 code, not part of this layer.
  22 new unit tests (13 coordinator, 9 integrity) cover the pure decay math, the weighted-attestation walk (direct/transitive/cycle/multi-root cases, mirroring `count_attestations_pure`'s own test shape), and `validate_credit_transfer`/`validate_credit_burn`'s content checks. All existing tests still pass (77 coordinator + 19 integrity = 96 total); both zomes build clean, natively and for `wasm32-unknown-unknown`. `hdk`'s `unstable-countersigning` feature (now enabled in `dna/coordinator/Cargo.toml`) is real and shipped as of hdk 0.4.4, but is still labeled unstable by the Holochain project itself — worth knowing before bumping the hdk pin.
  **Verified live, and the live pass is the interesting part: it found three real defects that 96 passing unit tests could not.** `scripts/live-verify/credit-transfer.mjs` drives the whole flow against a real `hc sandbox` conductor hosting two agents (countersigning needs both signers on the same network, which two cells of one DNA on one conductor already are — so unlike federation's two-sandbox verification, this deliberately uses one conductor and two agents): a plain, non-countersigned commit is rejected by DHT-side validation, the real propose → accept (both sides) → exchange responses → finalize (both sides) → index → `get_credit_balance` round trip succeeds, and a single-signer `CreditBurn` debits the balance. All checks pass, ending with agent 1 at `-10`, agent 2 at `+10` from a transfer neither could have committed alone, and agent 1 at `-13` after burning 3. What it found, all three of which compiled clean and passed every unit test: (1) **`create_entry` can never produce a countersigned entry** — the app must build `Entry::CounterSign` itself from every party's signed `PreflightResponse` and commit it via `create`; the original code's stated reasoning that "HDK recognizes the already-accepted session" and that "`CreateInput` carries no countersigning field" was false in both halves, and the commit *succeeded as a plain single-signer entry*, caught only by this DNA's own integrity `validate` — the countersigning requirement held while the mechanism meant to satisfy it never ran; (2) **a chain in a countersigning session accepts the session entry and nothing else**, so the agent-anchor index link had to move into its own `link_credit_transfer` call made after the session releases the chain; (3) **`to_app_option` silently drops countersigned entries** — it matches `Entry::App` alone and returns `Ok(None)`, not an error, for `Entry::CounterSign`, so with the first two fixed, transfers committed and validated correctly and every balance still read exactly 0, indistinguishable from an agent who had never transacted. All three live where this codebase's pure, host-call-free unit tests structurally cannot reach: in what the *host* does with an entry, not in what the app computes. **The burn-extension path was then verified too, found unreachable, and removed.** `scripts/live-verify/burn-friction.mjs` drives it against a clean conductor. `create_critique` is the only caller of `create_synaptic_link`, so a `SynapticLink` can only be created alongside a `Critique`; both carry a 20-per-hour budget, one call advances both, and `create_critique` checks the *Critique* budget first — which is a hard 20 with no burn tier. The `SynapticLink` burn tier began at exactly 20, the same count at which creating another Critique had already become impossible. Measured: 20 critiques succeed (creating 20 `SynapticLink`s), the 21st is refused by the Critique budget rather than the `SynapticLink` one, and burning 50 credit — ten times the 5.0 the 21st link would have cost — changed nothing. **It was then removed rather than repaired, because it was not merely unreachable but a net loss:** the one client that could reach it (one hand-crafting `CreateLink` actions outside `create_critique`) got ten extra links from it, paid for with burns that nothing funds, since balance is unenforceable at validation time. A plain hard limit of 20 is strictly stricter against exactly that client, and simpler — two constants and a burn-summing chain scan came out of each zome. `SynapticLink` friction is once again an absolute 20 per rolling hour with no way to buy past it. **The ledger itself is untouched and still live-verified** — `MutualCreditTransfer`, `CreditBurn`, countersigning and `get_credit_balance` all remain, and remain the substrate a real cost coupling would be rebuilt on once per-pair credit limits make a burn cost something. The consequence, stated plainly rather than left to be inferred: **the currency layer currently has no consumer** — nothing in this protocol reads `get_credit_balance` to make a decision. That was already true while the coupling was unreachable; removing it revealed the situation rather than creating it. See the docs brief §4.3 and §7.2.

...
- [ ] Worldline binding: populate `trace_payload` and `binding_key` on `WorldlineTrace` (temporal index — "when did this agent speak"; see §2.5)
- [ ] Neighborhood binding: bind a claim + its evidence chain + its critique stack as its own corpus (associative index — "what's near this claim") — a distinct use case from worldline binding, not a consequence of it; any recall API here must satisfy the constraint table in §2.5

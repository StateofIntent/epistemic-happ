...
- [ ] **Surface the epistemic state the backend already computes — now 21 of 57 coordinator functions, from 4.** Everything the protocol computes *about* discourse is what distinguishes it from a forum, and most of it had no surface. Shipped so far: the HUD layer (`get_discourse_health`, `get_effective_conductance`, `get_cross_domain_critiques`, `get_antibody_patterns_for`, `get_synaptic_link_friction_status`), membranes and governance (`get_membranes`, `get_membrane_members`, `join_membrane`, `create_membrane`, `publish_constitution`), evidence and grounding (`create_evidence`, `get_grounding_path`), and retraction in both directions (`create_retraction`, `get_retractions_for_claim`).

  **Still unsurfaced, in the order I'd take them:** the critique taxonomy (`get_all_critique_species`, `get_critique_species_adoption_count`, `create_critique_species`) — the evolving vocabulary of critique types is entirely invisible, and `domains/bootstrap.mjs` already puts 11 real `CritiqueSpecies` on the DHT with real parent/child structure that no screen can see; opt-in trust lenses (`is_agent_attested`, `grant_attestation`), which need care under §4.4 since they are the lenses a user aims; `assert_expertise`, which fits a UI that already publishes Claims precisely because expertise assertions *are* Claims rather than an unaccountable field; and the smaller `get_critiques_by_mode` and `get_agent_constitution`.

  **A caveat on the denominator, since 57 invites the wrong arithmetic.** The 36 unsurfaced functions are not 36 missing screens. Roughly nine are bridge-only (`create_mew`, `get_unbridged_claims`, `import_twitter_reply` and siblings) and belong to `bridge/`; nine are HRR/worldline, which Phase 3 defers with payloads empty, so there is nothing to show yet; two are `federation/`'s; `export_to_n4l` is an export path; and `attempt_unaccountable_membrane` is a negative-path prober whose own doc comment says it exists to *fail*. The genuinely UI-relevant remainder is roughly a dozen, which is what the list above enumerates. Tracking the raw ratio overstates the gap, and — as the read/write asymmetry above showed — can understate it too, since a function with a screen can still be half-surfaced.

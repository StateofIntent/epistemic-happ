...
Neither is a deep or complete taxonomy for its real field — each is a genuine, differentiated *starting point* a real founding community would extend, not a finished ontology. The point is that they're visibly different from each other and from `LumbarRehab`'s implicit vocabulary, not generic placeholders.
...
Both `climate.json` and `nutrition.json` were run for real against a live `hc sandbox` conductor with `bootstrap.mjs` — see the root `README.md`'s Phase 5 changelog for the full account of what was checked (every Constitution/Membrane/CritiqueSpecies hash resolving to real, correctly-parented entries; `get_all_critique_species` reflecting the real six-and-five-species taxonomies with correct `parent_species` links).

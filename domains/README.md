...
`compute-energy.json` has now had its live bootstrap run, against a Holochain 0.7 conductor: all ten `CritiqueSpecies` were created, and reading the taxonomy back off the DHT afterwards gives ten species — four roots and six whose `parent_species` resolves to a real species created earlier in the same run, with no dangling parents. Both `climate.json` and `nutrition.json` were run for real against a live `hc sandbox` conductor with `bootstrap.mjs` — see the root `README.md`'s Phase 5 changelog for the full account of what was checked (every Constitution/Membrane/CritiqueSpecies hash resolving to real, correctly-parented entries; `get_all_critique_species` reflecting the real six-and-five-species taxonomies with correct `parent_species` links).

...
| `get_attestation_weight` | `GetAttestationWeightPayload { candidate: AgentPubKey, membrane: AnyDhtHash, policy: WeightedAttestationPolicy }` | `f32` |
...
`WeightedAttestationPolicy { roots: Vec<AgentPubKey>, max_depth: u8, transitivity_decay: f32, epoch_half_life_secs: f64 }` — a graded version of the same walk, over the same two attestation sources, with the same visit cap and cycle guard. Each attestation contributes its own age-decayed strength (half-life `epoch_half_life_secs`; non-positive means no age decay), multiplied by `transitivity_decay` (clamped to `[0.0, 1.0]`) once per hop crossed to reach it.

Precisely, what this computes is a **caller-scoped, age- and distance-decayed reachability sum** over the attestation graph. Two mechanisms are borrowed from MeritRank (arXiv 2207.09950) but it is not an implementation of it, and an implementer MUST NOT treat it as conferring MeritRank's Sybil tolerance: MeritRank defines three decay mechanisms (transitivity, connectivity, epoch) and credits transitivity plus **connectivity** with that property, while finding epoch decay reduces it. This omits connectivity decay. It is also unnormalized — unlike the PageRank-family algorithms it resembles, an agent with more attesters scores strictly higher without bound — and its result depends on link iteration order, since the shared `visited` set gives the first path to reach a node the whole of that node's contribution and every later path zero.

`roots` is not optional here: a weighted walk with no roots has nothing to be weighted relative to. That is the point. **The returned `f32` is meaningful only relative to the roots and decay parameters the caller passed in** — it is computed on request, never stored, never a default, and never comparable between two callers who chose different roots. A conforming implementation MUST NOT store, cache as authoritative, or expose this as a per-agent score; doing so would construct exactly the canonical, comparative reputation number Invariant #1 (§7) forbids, which is why the two systems this design is nearest to (SourceCred, EigenTrust) were studied and deliberately not adopted.
